...
function fleetTonDashboardCard(rows, fleet) {
  ...
  if (!validRows.length) return `<article class="fleet-ton-card fleet-ton-${tone}"><div class="fleet-ton-card-heading"><span>${escapeHtml(label)}</span><small>0 embarques com R$/ton</small></div><div class="fleet-ton-empty">Sem valores válidos para esta frota.</div></article>`;
  const lowest = finiteMinimum(validRows, 'costPerTon'), highest = finiteMaximum(validRows, 'costPerTon'), lowestCount = validRows.filter(row => row.costPerTon === lowest).length, highestCount = validRows.filter(row => row.costPerTon === highest).length;
  const lowWidth = Math.max(14, Math.min(100, highest > 0 ? lowest / highest * 100 : 100));
  const metric = (kind, value, count, width, level) => `<button class="fleet-ton-metric fleet-ton-${level}" type="button" data-financial-kind="fleet-ton-${kind}" data-financial-fleet="${escapeHtml(fleet)}" aria-label="Abrir embarques com ${kind === 'low' ? 'menor' : 'maior'} R$/ton de ${escapeHtml(label)}"><span class="fleet-ton-metric-label">${kind === 'low' ? 'Menor' : 'Maior'}</span><span class="fleet-ton-track" aria-hidden="true"><span class="fleet-ton-fill" style="width:${Math.round(width * 10) / 10}%"></span></span><strong>${money(value)}<small>/ton</small></strong><em>${count} embarque${count === 1 ? '' : 's'} neste valor</em></button>`;
  return `<article class="fleet-ton-card fleet-ton-${tone}"><div class="fleet-ton-card-heading"><span>${escapeHtml(label)}</span><small>${validRows.length} embarque${validRows.length === 1 ? '' : 's'} com R$/ton</small></div><div class="fleet-ton-chart">${metric('low', lowest, lowestCount, lowWidth, 'low')}${metric('high', highest, highestCount, 100, 'high')}</div><button class="fleet-ton-view-all" type="button" data-financial-kind="fleet-ton-all" data-financial-fleet="${escapeHtml(fleet)}">Ver todos de ${escapeHtml(label)} →</button></article>`;
}
function renderFleetTonDashboard(rows) { const dashboard = $('fleetTonDashboard'); if (dashboard) dashboard.innerHTML = FLEET_ORDER.map(fleet => fleetTonDashboardCard(rows, fleet)).join(''); }
...
function openFinancialDetail(kind = 'all', filter = '', fleet = '', category = '') {
  ...
  if (kind === 'fleet-ton-all' || kind === 'fleet-ton-low' || kind === 'fleet-ton-high') {
    ...
    if (kind === 'fleet-ton-low') { rows = rows.filter(row => row.costPerTon === lowest); title = `Menor R$/ton — ${label}`; subtitle = `Embarques de ${label} que formam exatamente o menor valor do período`; detailMetric = Number.isFinite(lowest) ? `${money(lowest)}/ton` : 'Sem R$/ton'; }
    else if (kind === 'fleet-ton-high') { rows = rows.filter(row => row.costPerTon === highest); title = `Maior R$/ton — ${label}`; subtitle = `Embarques de ${label} que formam exatamente o maior valor do período`; detailMetric = Number.isFinite(highest) ? `${money(highest)}/ton` : 'Sem R$/ton'; }
    else { title = `R$/ton — ${label}`; subtitle = `Todos os embarques de ${label}, do menor para o maior R$/ton`; detailMetric = rows.length ? `Média ${money(finiteAverage(rows, 'costPerTon'))}/ton` : 'Sem R$/ton'; }
  }
  ...
  const tonKinds = ['spot','ton-average','ton-best','ton-highest','fleet-ton-all','fleet-ton-low','fleet-ton-high'], total = detailMetric || (kind === 'cost' ? money(finiteSum(rows, 'cost')) : kind === 'revenue' ? money(finiteSum(rows, 'revenue')) : tonKinds.includes(kind) ? `Média ${moneyOrDash(finiteAverage(rows, 'costPerTon'))}/ton` : `${rows.length} embarque${rows.length === 1 ? '' : 's'}`);
